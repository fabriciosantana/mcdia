...
A prova de conceito usa TF--IDF com unigramas e bigramas e similaridade do
cosseno. Foram elaboradas dez perguntas a partir dos resumos oficiais, cada uma
associada a um pronunciamento conhecido, enquanto somente os textos integrais
formaram o índice. O experimento mede busca de item conhecido por Hit@$k$ e
\textit{Mean Reciprocal Rank} (MRR). Ele não estima \textit{recall} temático,
pois outros documentos potencialmente relevantes ainda não foram julgados. A
avaliação futura da geração deverá examinar correção das citações, cobertura da
pergunta, fidelidade aos trechos e capacidade de abstenção.
...
No baseline, o pronunciamento conhecido foi recuperado na primeira posição em
20\% das dez consultas, nas cinco primeiras posições em 40\% e nas dez
primeiras também em 40\%. O MRR foi 0,301 e a mediana da melhor posição foi
17,5. O resultado demonstra a viabilidade do fluxo, mas também mostra que a
correspondência lexical isolada é insuficiente para várias formulações. Isso
fundamenta a comparação posterior com recuperação vetorial e fusão híbrida, sem
pressupor que maior similaridade semântica garantirá relevância.
...
referência deverá receber julgamentos adicionais, seguidos da comparação entre
recuperação lexical, vetorial e híbrida e da definição dos critérios de
abstenção. A adoção institucional dependerá ainda de
