...
\begin{table}[H]
\centering
\caption{Comparação de estudos diretamente relacionados ao protocolo}
\label{tab:trabalhos-relacionados}
\small
\begin{tabular}{p{2.7cm}p{2.8cm}p{3.1cm}p{2.5cm}}
\toprule
Estudo & Corpus/domínio & Foco de avaliação & Transferência para o Senado \\
\midrule
ARES & QA em múltiplos domínios & Groundedness, correção e completude & Rubrica geral; requer calibração local \\
LegalBench-RAG & Questões jurídicas em inglês & Recuperação e resposta fundamentada & Alta proximidade jurídica; baixa proximidade institucional \\
Trautmann et al. & QA jurídico & Groundedness por similaridade, NLI e LLM & Base para fidelidade ao contexto \\
RAGTruth & Respostas RAG em vários domínios & Alucinação em nível de caso e palavra & Taxonomia de erros; idioma não parlamentar \\
Zhang et al. & Texto gerado com citações & Suporte completo, parcial ou ausente & Base para verificabilidade graduada \\
GaRAGe & QA com passagens anotadas & Grounding relevante e deflexão & Base para perguntas não respondíveis \\
Reuter et al. & Grandes bases jurídicas & Erro no nível do documento e chunking & Hipótese de erro documental a testar \\
eRAG & Três conjuntos de QA & Utilidade downstream de cada documento & Liga recuperação ao desempenho da resposta \\
GroUSE & QA fundamentado controlado & Meta-avaliação por testes unitários & Calibra o LLM-as-a-judge \\
\bottomrule
\end{tabular}
\end{table}
...
A literatura converge em três pontos: recuperação e geração devem ser separadas analiticamente; suporte documental precisa ser verificado em unidade menor que a resposta inteira; e a avaliação automática requer calibração e limites explícitos. A comparação das fontes centrais também recomenda separar três unidades de erro: documento recuperado, trecho recuperado e afirmação gerada. Reuter et al. sustentam a primeira distinção, LegalBench-RAG a segunda e RAGTruth e Zhang et al. a terceira, enquanto GroUSE e ConsJudge impõem cautela na interpretação de avaliadores automáticos. Essa decomposição orienta a matriz de análise da dissertação e evita que uma nota agregada oculte falhas de recuperação ou de atribuição. Ela diverge quanto à unidade ideal de avaliação, à definição de relevância e ao equilíbrio entre conhecimento paramétrico e contexto recuperado. Os estudos selecionados também se concentram em inglês, legislação ou domínios gerais.
