...
\input{resultados/indicadores.tex}
...
administradas pelo órgão \cite{rfbDados,rfbDicionario}. Este trabalho examina a
completude dessa publicação como requisito para seu uso em análises e
relatórios de auditoria. A proposta se insere na governança da informação
tributária, cuja relevância decorre da necessidade
...
contexto de uso \cite{wang1996}. Para comparar as medidas, adotou-se uma
contagem simples de vazios como referência experimental, sem caracterizá-la
como prática vigente na Receita. O objeto da análise é a qualidade da
publicação agregada; a identificação de contribuintes, a estimação de sonegação
e a avaliação de desempenho de unidades administrativas estão fora do escopo.
...
A abordagem permite relacionar cada classificação a uma regra e a uma data
verificáveis. A aplicação determinística dos intervalos documentados torna
o procedimento passível de reprodução e inspeção. O registro das regras e dos indicadores
...
O procedimento de aquisição obteve diretamente do repositório oficial o CSV e seu dicionário em PDF,
...
Os 13 cenários controlados produziram as classes de ocorrência esperadas. O
procedimento gerou uma tabela de ocorrências com chave, coluna, regra e valor
original, acompanhada da classificação de todas as células do recorte. Na base
observada, a tabela ficou sem ocorrências segundo os controles definidos.
Propõe-se utilizar esse controle na verificação de novas versões antes de incorporá-las a relatórios e
